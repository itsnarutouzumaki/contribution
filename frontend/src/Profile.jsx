...
function RankBar({ User, status, rank }) {
  ...
  const deltaColor = delta > 0 ? "text-green-900 ml-1 font-black" : "text-red-900 ml-1 font-black";
  ...
  return (
    <a
      className={`w-[90%] hover:w-full border-2 my-1 py-1 bg-blue-200 text-black hover:bg-blue-700 hover:text-white hover:shadow-2xl hover:shadow-red-950 duration-250 hover:font-black ${
        status
          ? "border-green-50 bg-green-200 font-black sticky top-12"
...
function Profile({ user }) {
  ...
  useEffect(() => {
    if (issueResponse) {
      setRank(last30 ? issueResponse.sortedScoresLast30 : issueResponse.sortedScoresAllTime);
    }
